...
d.add() #- добавляет элемент в конце списка
...
a = d.pop('1')  #- переменной присвоится значение ключа '1'
...
{word_id:word for (word,word_id) in name_dict.items()} # меняем в нашем словаре ключи со значениями
